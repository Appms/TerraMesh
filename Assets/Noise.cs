...
public static class Noise {
	...
	public static NoiseMethod[] billowedMethods = {
		Billowed1D,
		Billowed2D,
		Billowed3D
	};

	public static NoiseMethod[] ridgedMethods = {
		Ridged1D,
		Ridged2D,
		Ridged3D
	};
	...
	public static NoiseSample Billowed1D( Vector3 point, float frequency){
	
		NoiseSample sample = new NoiseSample ();
		NoiseSample perlinSample = Perlin1D (point, frequency);

		sample.value = Mathf.Abs(perlinSample.value);
		if (sample.value == perlinSample.value) {
			sample.derivative.x = perlinSample.derivative.x;
			sample.derivative.y = perlinSample.derivative.y;
			sample.derivative.z = perlinSample.derivative.z;
		} else {
			sample.derivative.x = -1 * perlinSample.derivative.x;
			sample.derivative.y = -1 * perlinSample.derivative.y;
			sample.derivative.z = -1 * perlinSample.derivative.z;
		}
	...
	public static NoiseSample Billowed2D( Vector3 point, float frequency){
		
		NoiseSample sample = new NoiseSample ();
		NoiseSample perlinSample = Perlin2D (point, frequency);
		
		sample.value = Mathf.Abs(perlinSample.value);
		sample.derivative = -1 * perlinSample.derivative;
		/*sample.derivative.x = Mathf.Abs(perlinSample.derivative.x);
		sample.derivative.y = Mathf.Abs(perlinSample.derivative.y);
		sample.derivative.z = Mathf.Abs(perlinSample.derivative.z);*/
		
		return sample;
		
	}

	public static NoiseSample Billowed3D( Vector3 point, float frequency){
		
		NoiseSample sample = new NoiseSample ();
		NoiseSample perlinSample = Perlin3D (point, frequency);
	...
	public static NoiseSample Ridged1D( Vector3 point, float frequency){
		
		return 1.0f - Billowed1D(point, frequency);
		
	}
	
	public static NoiseSample Ridged2D( Vector3 point, float frequency){
		
		return 1.0f - Billowed2D(point, frequency);
		
	}
	
	public static NoiseSample Ridged3D( Vector3 point, float frequency){
		
		return 1.0f - Billowed3D(point, frequency);
